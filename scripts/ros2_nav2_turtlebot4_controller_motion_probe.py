#!/usr/bin/env python3
"""Opt-in TurtleBot4 controller/physics motion probe.

This diagnostic publishes a tiny raw velocity command directly to the simulator
controller input and checks whether `/odom` changes. It is not a MissionOS
dispatch path, does not send a Nav2 goal, and must not claim completion.
"""
...
ROS2_NAV2_CONTROLLER_MOTION_PROBE_ENV = (
    "RUN_MISSIONOS_ROS2_NAV2_CONTROLLER_MOTION_PROBE"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_TARGET_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_TARGET_TOPIC"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_ODOM_ENV = "ROS2_NAV2_CONTROLLER_MOTION_PROBE_ODOM_TOPIC"
ROS2_NAV2_CONTROLLER_MOTION_PROBE_LINEAR_X_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_LINEAR_X_MPS"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_ANGULAR_Z_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_ANGULAR_Z_RADPS"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_DURATION_ENV = (
...
ROS2_NAV2_CONTROLLER_MOTION_PROBE_SETTLE_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_SETTLE_S"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_THRESHOLD_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_MOTION_THRESHOLD_M"
)
ROS2_NAV2_CONTROLLER_MOTION_PROBE_STOP_PUBLISHES_ENV = (
    "ROS2_NAV2_CONTROLLER_MOTION_PROBE_STOP_PUBLISHES"
)
...
def run_probe() -> dict[str, Any]:
    try:
        ...
        return _base_summary(
            ran=False,
            reason="ros2_controller_motion_probe_python_dependencies_missing",
    ...
    odom_topic = os.environ.get(ROS2_NAV2_CONTROLLER_MOTION_PROBE_ODOM_ENV, "/odom")
    linear_x_mps = _float_env(ROS2_NAV2_CONTROLLER_MOTION_PROBE_LINEAR_X_ENV, -0.08)
    ...
    stop_publishes = max(
        _int_env(ROS2_NAV2_CONTROLLER_MOTION_PROBE_STOP_PUBLISHES_ENV, 5),
        0,
    )
    ...
    node = rclpy.create_node("missionos_ros2_nav2_controller_motion_probe")
...
def main() -> int:
    if not _truthy_env(ROS2_NAV2_CONTROLLER_MOTION_PROBE_ENV):
        print(
            json.dumps(
                _base_summary(
                    ran=False,
                    reason=(
                        f"{ROS2_NAV2_CONTROLLER_MOTION_PROBE_ENV} is not set "
                        "to 1; no diagnostic raw velocity was published."
                    ),
                    diagnostic_raw_velocity_published=False,
                    target_control_topic_published=False,
                    robot_motion_observed=False,
                    blocking_reasons=["controller_motion_probe_opt_in_not_enabled"],
                ),
                ensure_ascii=True,
                indent=2,
                sort_keys=True,
