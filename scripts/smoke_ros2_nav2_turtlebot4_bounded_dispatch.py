#!/usr/bin/env python3
"""Opt-in MissionOS -> ROS2/Nav2 TurtleBot4 bounded dispatch smoke."""
...
def main() -> int:
    if not _truthy_env(ROS2_NAV2_BOUNDED_DISPATCH_SMOKE_ENV):
        print(
            json.dumps(
                {
                    "smoke": "ros2_nav2_turtlebot4_bounded_dispatch",
                    "ran": False,
                    "reason": (
    ...
    adapter = Ros2Nav2HardwareAdapter(
        config=Ros2Nav2HardwareAdapterConfig(
            ...
            goal_pose=Nav2GoalPose(
                ...
                x_m=float(os.environ.get("ROS2_NAV2_GOAL_X_M", "0.25")),
                ...
                max_speed_mps=0.25,
                max_distance_m=3.0,
                label="turtlebot4_short_nav2_goal",
            ),
            execution_mode=HardwareExecutionMode.SIM,
            operator_approval_ref="smoke_operator_approval_nav2_turtlebot4_001",
    ...
    if evidence.physical_execution_invoked:
        raise SystemExit("Nav2 TurtleBot4 smoke claimed physical execution")
    if evidence.dispatch_request_sent and evidence.completion_scope not in {
        "sim_action",
        "none",
    }:
        raise SystemExit("Nav2 TurtleBot4 smoke used an invalid completion scope")
