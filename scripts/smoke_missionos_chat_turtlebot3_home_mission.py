#!/usr/bin/env python3
"""Runtime smoke for MissionOS chat -> TurtleBot3 home mission routing.

Default mode starts a real Gateway on loopback, sends chat plan/approve/run
HTTP requests, and verifies the run blocks before ROS2 dispatch because no
bridge is opted in. Set RUN_MISSIONOS_CHAT_TURTLEBOT3_HOME_MISSION_SMOKE_WITH_BRIDGE=1
and provide the ROS2/Nav2 bridge env to exercise a real simulator bridge.
"""
...
def main() -> int:
    ...
    if low_battery["recovery_execution_permitted_by_envelope"] is not True:
        raise SystemExit("low-battery return_home proposal was not classified executable")
    if low_battery["recovery_dispatch_request_sent"] is not False:
        raise SystemExit("low-battery smoke dispatched recovery without recovery bridge")
