#!/usr/bin/env python3
"""Opt-in Gazebo telemetry-only Docker smoke.

This smoke starts a Dockerized external process that behaves like a Gazebo-side
telemetry/log source. Mission OS reads stdout logs only, sanitizes the latest
telemetry sample, builds HIL review/gate artifacts, attaches them to a
temporary task, and verifies the read-only Control UI can render the result.
"""
...
def _wait_for_telemetry(*, timeout_seconds: float = 60.0) -> str:
    ...
    while time.monotonic() < deadline:
        logs = _logs()
        if (
            "GAZEBO_TELEMETRY_SOURCE_READY" in logs
            and "PX4_GAZEBO_TELEMETRY " in logs
        ):
            return logs
        time.sleep(1)
    raise RuntimeError("Gazebo-compatible telemetry log did not appear")
...
def _render_control_ui_task(stored_task: dict) -> dict:
    ...
    return {
        ...
        "has_source": "gazebo-compatible-log-source" in html,
...
def _exercise_logs() -> dict:
    ...
    sanitized = collect_gazebo_log_sanitized(logs)
    ...
    with TemporaryDirectory() as tmp:
        ...
        task = store.create(
            ...
            title="Gazebo telemetry-only smoke",
            status="running",
            artifacts={"existing": {"kept": True}},
        )
        artifacts = attach_gazebo_log_smoke_artifacts(
            task["task_id"],
            logs,
    ...
    assert sanitized.source_kind == "gazebo_compatible_log_source"
    assert sanitized.source_id == "gazebo-compatible-log-source"
    ...
    return {
        ...
        "profile": PROFILE,
        "gazebo_or_compatible_process_started": True,
