"""Runtime smoke for delivery mission policy review."""
...
def _contract():
    return build_delivery_mission_contract(
        mission_id="delivery-policy-smoke",
        pickup_location={
...
def main() -> int:
    telemetry = sanitize_px4_gazebo_telemetry_sample(
        {
            "sample_id": "delivery-policy-smoke",
            "source": {
                "source_kind": "px4_sih_stdout_log",
                "source_id": "px4-sih-delivery-policy",
                "vehicle_id": "vehicle-delivery-smoke",
            },
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        task = store.create(
            ...
            title="Delivery policy smoke",
            status="running",
            artifacts={"existing": {"schema_version": "existing.v1"}},
        )
        artifacts = attach_delivery_mission_policy_review(
            task["task_id"],
    ...
    review = artifacts["delivery_mission_policy_review"]
    result = {
        "schema_version": review["schema_version"],
        "passed": review["passed"],
        "status": review["status"],
        "warning_reasons": review["warning_reasons"],
        "return_to_home_recommended": review["return_to_home_recommended"],
        "abort_recommended": review["abort_recommended"],
        ...
        "ros_dispatch_allowed": review["ros_dispatch_allowed"],
        "mavlink_dispatch_allowed": review["mavlink_dispatch_allowed"],
        "actuator_execution_allowed": review["actuator_execution_allowed"],
    }
