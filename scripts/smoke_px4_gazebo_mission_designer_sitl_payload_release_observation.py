"""Runtime smoke for Mission Designer SITL payload-release observation.
...
not synthesize flight or payload-release facts.
"""
...
OPT_IN_ENV = "RUN_MISSION_DESIGNER_SITL_PAYLOAD_RELEASE_OBSERVATION_SMOKE"
SUMMARY_PATH_ENV = "MISSION_DESIGNER_SITL_PAYLOAD_RELEASE_OBSERVATION_SUMMARY_PATH"
...
def _load_horizontal_summary() -> dict[str, Any]:
    ...
    if summary.get("payload_release_observed") is not True:
        raise RuntimeError("payload release smoke requires observed payload release")
    if (
        summary.get("payload_release_event_source")
        != "gazebo_detachable_joint_detach_event"
    ):
        raise RuntimeError(
            "payload release smoke requires Gazebo detachable-joint event"
        )
    for key in (
        ...
        if summary.get(key) in ("", None):
            raise RuntimeError(f"payload release smoke requires {key}")
    return summary
...
async def _main() -> dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="mission-designer-sitl-payload-release-"
        ) as tmp:
                ...
                async with httpx.AsyncClient(base_url=base_url, timeout=20.0) as client:
                    ...
                    prepared = await _post_json(
                        ...
                        "/px4-gazebo/mission-scenarios/prepare-sitl-execution",
                        {
                            ...
                            "owner_session_id": "smoke-sitl-payload-release",
                    ...
                    task_id = prepared["summary"]["task_id"]
                    task_record = gateway.task_store.get(task_id)
                    if task_record is None:
                        raise RuntimeError(
                            "Gateway task missing before live-run attach"
                        )
                    ...
                    payload_attached = attach_px4_gazebo_mission_designer_sitl_payload_release_observation(
                        task_id,
                        horizontal_summary=live_summary,
                        task_store_factory=lambda: gateway.task_store,
                    )
                    stored_response = await client.get(f"/tasks/{task_id}")
        ...
        observation = payload_attached[
            "px4_gazebo_mission_designer_sitl_payload_release_observation"
        ]
        live_run = live_attached["px4_gazebo_mission_designer_sitl_live_flight_run"]
        release_event = payload_attached["px4_gazebo_sitl_payload_release_event"]
        ...
        if (
            ...
            raise RuntimeError(
                "stored task missing payload release observation artifact"
            )
        for key in (
        ...
        if observation["payload_release_observed"] is not True:
            raise RuntimeError("payload release observation did not observe release")
        if observation["dropoff_verified"] is not False:
            raise RuntimeError("payload release observation verified dropoff")
        if observation["payload_release_event_ref"] != (
            f"px4_gazebo_sitl_payload_release_event:{release_event['event_id']}"
        ):
            raise RuntimeError("payload release event ref mismatch")
        live_run_ref = (
        ...
        if observation["live_flight_run_ref"] != live_run_ref:
            raise RuntimeError("payload release observation live-run ref mismatch")
        if observation["mission_item_binding_sha256"] != (
            live_run["mission_item_binding_sha256"]
        ):
            raise RuntimeError("payload release observation mission item mismatch")
        if observation["payload_release_bound_to_live_run"] is not True:
            raise RuntimeError("payload release observation not bound to live run")

        return {
            "mission_designer_sitl_payload_release_observation_smoke_passed": True,
            ...
            "payload_release_observation_schema_version": observation["schema_version"],
            ...
            "payload_release_bound_to_live_run": observation[
            ...
            "execution_result_payload_release_observed": result[
                "payload_release_observed"
            ],
            "payload_release_observed": observation["payload_release_observed"],
            "payload_release_event_verified": observation[
                "payload_release_event_verified"
            ],
            "dropoff_verified": result["dropoff_verified"],
            "observation_dropoff_verified": observation["dropoff_verified"],
