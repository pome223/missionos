...
attest the physical bench state via the environment:
    ...
    PX4_VEHICLE_SECURED=1                   # airframe is restrained
    PX4_PHYSICAL_ESTOP_AVAILABLE=1          # tested stop is within reach
    PX4_POWER_DISCONNECT_AVAILABLE=1        # immediate power cut is available
...
Run (operator, at the bench)::
    ...
    RUN_PX4_REAL_HARDWARE_SERIAL_SMOKE=1 PX4_SERIAL_DEVICE=/dev/tty.usbmodem1234 \
        ...
        PX4_OPERATOR_PRESENT=1 PX4_VEHICLE_SECURED=1 \
        PX4_PHYSICAL_ESTOP_AVAILABLE=1 PX4_POWER_DISCONNECT_AVAILABLE=1 \
...
def main() -> int:
    ...
    if not all(
        ...
        raise SystemExit(
            ...
            "attestation: require propellers removed, operator present, vehicle "
            "secured, physical E-stop, and immediate power disconnect"
        )
