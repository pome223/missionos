...
def test_payload_recovery_audit_keeps_delivery_claim_closed() -> None:
    summary = {
        "delivery_completion_claimed": False,
        "hardware_target_allowed": False,
        "physical_execution_invoked": False,
        "payload_feasibility_advisory_ref": "payload_feasibility_advisory:1",
        "payload_advisory_consumed_by_ref": "payload_recovery_action:1",
        "payload_recovery_dispatch_status": "accepted",
        "payload_recovery_completed": True,
        "payload_recovery_state_observed": True,
        "payload_recovery_state_label": "return_to_launch_state_observed",
    }
    expectations = audit.PayloadRecoveryAuditExpectations(
        advisory_ref_prefix="payload_feasibility_advisory:",
        payload_action_ref="payload_recovery_action:1",
        payload_action="rtl",
        landing_z_threshold_m=0.15,
    )

    audit.audit_payload_recovery_summary(summary, expectations=expectations)
    summary["delivery_completion_claimed"] = True
    with pytest.raises(AssertionError):
        audit.audit_payload_recovery_summary(summary, expectations=expectations)


def test_route_deviation_audit_requires_observed_state_not_only_ack() -> None:
    summary = {
        "final_status": "recovered_rtl",
        "task_status": "completed",
    ...
    expectations = audit.RouteDeviationRecoveryAuditExpectations(
        on_deviation_action="rtl",
        post_recovery_action="none",
        landing_z_threshold_m=0.15,
    )

    audit.audit_route_deviation_recovery_summary(
        summary,
...
def test_post_recovery_land_requires_its_own_observation() -> None:
    summary = {
        "final_status": "post_recovery_recovered_land",
        "task_status": "completed",
        "recovery_completed": True,
        "recovery_state_observed": True,
        "recovery_state_label": "return_to_launch_state_observed",
        "post_recovery_dispatch_status": "accepted",
        "post_recovery_completed": True,
        "post_recovery_state_observed": True,
        "post_recovery_pose_z_m": 0.1,
        "existing_artifacts_retained": True,
        "deviation_samples": [{"deviation_xy_m": 2.0}],
        "delivery_completion_claimed": False,
        "hardware_target_allowed": False,
    ...
    summary["post_recovery_state_observed"] = False
    with pytest.raises(AssertionError):
        audit.audit_route_deviation_recovery_summary(
            summary,
            expectations=expectations,
