...
from src.runtime.px4_gazebo_route import environmental_realism
...
def test_legacy_route_entrypoint_uses_packaged_world_builders() -> None:
    ...
    assert (
        environmental_realism.visibility_marker_fog_element is world.visibility_marker_fog_element
    )
...
def test_world_model_fragments_are_valid_xml_with_stable_identities() -> None:
    fragments = {
        ...
        "mission_designer_traffic_conflict_marker": (world.traffic_conflict_world_sdf_patch()),
        "mission_designer_alternate_landing_marker": (world.alternate_landing_world_sdf_patch()),
...
def test_vehicle_plugin_and_wind_fragments_remain_valid_xml() -> None:
    ...
    assert detachable_joint.findtext("detach_topic") == ("/model/x500_0/delivery_payload/detach")

    wind = _fragment_root(world.wind_effects_world_sdf_patch(wind_x_mps=3.25, wind_y_mps=-1.5))
