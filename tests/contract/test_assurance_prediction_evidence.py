...
def test_dispatch_revalidates_original_prediction_against_current_owner(change):
    ...
    if change == "revision":
        context["state_revision"] = "revision2"
    if change == "missing":
        envelope = None
    if change == "changed":
        envelope["forecast"]["forecasts"][0]["risk_score"] = 0.1
    check = revalidate_incident_prediction(
        graph, envelope=envelope, current_context=context, now=now
    )
