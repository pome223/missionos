...
@dataclass(frozen=True)
class CompletedDeliveryArtifactChain:
    contract: Any
...
def build_completed_delivery_contract() -> Any:
    return build_delivery_mission_contract(
        mission_id="completed-delivery-contract-fixture",
        pickup_location={
            "location_id": "pickup-pad-a",
            "latitude": 35.681236,
            "longitude": 139.767125,
...
def build_completed_delivery_artifact_chain(
    *,
    store: TaskStore,
    task_id: str,
) -> CompletedDeliveryArtifactChain:
    contract = build_completed_delivery_contract()
    ...
    bounded_request = approved["bounded_simulation_request"]
