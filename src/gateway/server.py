...
from src.gateway.turtlebot3_task_lifecycle import (
    ...
    missionos_attach_turtlebot3_recovery_decision_summary as _missionos_attach_turtlebot3_recovery_decision_summary,
    missionos_instruction_requests_turtlebot3_home_mission as _missionos_instruction_requests_turtlebot3_home_mission,
    missionos_turtlebot3_home_mission_approval as _missionos_turtlebot3_home_mission_approval,
    missionos_turtlebot3_home_mission_has_proposal as _missionos_turtlebot3_home_mission_has_proposal,
    missionos_turtlebot3_home_mission_plan as _missionos_turtlebot3_home_mission_plan,
)
...
def _missionos_create_running_turtlebot3_home_mission_task(
    *,
    ...
    return create_running_turtlebot3_home_mission_task(
...
def _missionos_create_turtlebot3_home_mission_task(
    *,
    ...
    return create_turtlebot3_home_mission_task(
        task_store=get_task_store(),
...
def _missionos_mission_designer_context_message(context: Mapping[str, Any]) -> str:
    ...
    if _missionos_turtlebot3_home_mission_has_proposal(context):
        ...
        objective = str(
            ...
            or f"the current {robot_label} Nav2 mission proposal"
        )
        if isinstance(turtlebot3_execution, Mapping):
            ...
            return (
                f"The current {robot_label} Nav2 mission has execution evidence: {objective}. "
        ...
        if _missionos_turtlebot3_home_mission_approval(context):
            return (
                f"The current {robot_label} Nav2 mission is approved for one bounded Nav2 simulator leg: {objective}. "
    ...
    try:
        if context_error and intent in {"approve", "reject", "execute", "status", "clarification"}:
            ...
            if isinstance(
                ...
                robot_label = str(summary.get("robot_label") or "TurtleBot3")
