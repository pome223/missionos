"""Formal opt-in PX4/Gazebo horizontal pickup-to-dropoff runtime entrypoint.

This production harness remains simulator-only and requires the historical
explicit opt-in environment variable.  The compatibility smoke module aliases
this module so existing imports and monkeypatches retain identical semantics.
"""
...
def _refresh_payload_recovery_realism(
    *,
    payload_model_root: Path | None,
) -> _PayloadRecoveryRealismRefresh:
    global TELEMETRY_REALISM_SUMMARY, VEHICLE_REALISM_SUMMARY

    _refresh_battery_realism_observation_from_trace()
    TELEMETRY_REALISM_SUMMARY = _telemetry_observer_dropout_realism()
    VEHICLE_REALISM_SUMMARY = _vehicle_payload_mass_realism(payload_model_root=payload_model_root)
    return _PayloadRecoveryRealismRefresh(
        telemetry_summary=TELEMETRY_REALISM_SUMMARY,
        vehicle_summary=VEHICLE_REALISM_SUMMARY,
        wind_artifacts=_wind_realism_summary_artifacts(
            cleanup_status="teardown_required_after_summary"
        ),
        vehicle_artifacts=_vehicle_realism_summary_artifacts(),
    )


def _refresh_route_deviation_realism(
    *,
    run_dir: Path,
    route_start_xy_m: tuple[float, float],
    route_dropoff_xy_m: tuple[float, float],
) -> _RouteDeviationRealismRefresh:
    global TELEMETRY_REALISM_SUMMARY
    ...
    return _RouteDeviationRealismRefresh(
        route_blocking_verification_summary=dict(ROUTE_BLOCKING_VERIFICATION_SUMMARY or {}),
        vehicle_summary=dict(VEHICLE_REALISM_SUMMARY or {}),
        battery_summary=dict(BATTERY_REALISM_SUMMARY or {}),
        telemetry_summary=dict(TELEMETRY_REALISM_SUMMARY or {}),
        wind_artifacts=_wind_realism_summary_artifacts(
            cleanup_status="teardown_required_after_summary"
        ),
        vehicle_artifacts=_vehicle_realism_summary_artifacts(),
...
def _run_normal_route_deviation_handoff(
    *,
    store: TaskStore,
    task_id: str,
    run_dir: Path,
    ...
    recovery_allowlist: Any,
    recovery_dispatch: Any,
) -> Any:
    return _run_route_deviation_flow(
        _RouteDeviationFlowInputs(
            store=store,
            task_id=task_id,
            ...
            pickup_pose=pickup_pose,
            post_recovery_action=post_recovery_action,
            recovery_approval=recovery_approval,
            recovery_allowlist=recovery_allowlist,
            recovery_dispatch=recovery_dispatch,
            supervisor_loop_requested=supervisor_loop_requested,
            multi_condition_supervisor_requested=(multi_condition_supervisor_requested),
            wind_requested_profile=_wind_requested_profile(),
            observed_at=NOW,
        ),
        runtime=_RouteDeviationRuntime(
            build_deviation_abort=build_px4_gazebo_route_deviation_abort,
            observe_dispatched_recovery=_observe_dispatched_recovery,
            build_recovery_completion=build_px4_gazebo_route_recovery_completion,
            observe_recovery_state=_observe_recovery_state,
            dispatch_recovery=_dispatch_emergency_recovery,
            refresh_realism=lambda: _refresh_route_deviation_realism(
                run_dir=run_dir,
...
def _project_normal_route_terminal_realism(
    *,
    run_dir: Path,
    payload_model_root: Path | None,
    pickup_pose: dict[str, float],
    route_approval: Any,
    obstacle_supervisor_loop_requested: bool,
    ...
    _refresh_battery_realism_observation_from_trace()
    summaries = blocking_decision.decision_summaries
    if summaries:
        MOVING_ACTOR_POSE_SUMMARY = summaries.get("moving_actor_pose", {})
        MOVING_ACTOR_PROXIMITY_SUMMARY = summaries.get("moving_actor_proximity", {})
    ...
    ALTERNATE_LANDING_EXECUTION_SUMMARY = _alternate_landing_execution_realism(
        emergency_approval=terminal_action.alternate_approval,
        emergency_allowlist=terminal_action.alternate_allowlist,
        emergency_dispatch=terminal_action.alternate_dispatch,
        completed_pose=terminal_action.completed_pose,
        landing_samples=terminal_action.landing_samples,
    )
    ...
    TELEMETRY_REALISM_SUMMARY = _telemetry_observer_dropout_realism()
    obstacle_supervisor_loop = (
        _build_obstacle_supervisor_loop() if obstacle_supervisor_loop_requested else None
    )
    RTH_BEHAVIOR_SUMMARY = _rth_behavior_execution_realism(
        emergency_approval=terminal_action.rth_approval,
        emergency_allowlist=terminal_action.rth_allowlist,
        emergency_dispatch=terminal_action.rth_dispatch,
        ...
        rth_samples=terminal_action.rth_samples,
    )
    _refresh_horizontal_contact_topic_summary(run_dir)
    return _NormalRouteTerminalProjection(
        payload_release_summary=(
            None if PAYLOAD_RELEASE_SUMMARY is None else dict(PAYLOAD_RELEASE_SUMMARY)
        ),
        telemetry_summary=dict(TELEMETRY_REALISM_SUMMARY or {}),
        vehicle_summary=dict(VEHICLE_REALISM_SUMMARY or {}),
        obstacle_supervisor_recovery_loop=obstacle_supervisor_loop,
        wind_artifacts=_wind_realism_summary_artifacts(
            cleanup_status="teardown_required_after_summary"
        ),
        vehicle_artifacts=_vehicle_realism_summary_artifacts(),
...
def _reset_route_runtime_state(run_dir: Path) -> Path:
    trace_path = run_dir / "pose_samples.jsonl"
    trace_path.write_text("")
    namespace = globals()
    namespace["LIVE_POSE_TRACE_PATH"] = trace_path
    for name in (
        "PAYLOAD_RELEASE_SUMMARY",
...
def _initialize_route_runtime_realism(
    payload_model_root: Path | None,
) -> dict[str, Any]:
    global \
        TERRAIN_WORLD_REALISM_SUMMARY, \
    ...
    initial = _collect_initial_realism(
        _PX4RouteInitialRealismRuntime(
            ...
            mavlink_link_realism=_mavlink_link_degradation_realism,
        ),
        payload_model_root=payload_model_root,
    )
    TERRAIN_WORLD_REALISM_SUMMARY = initial.terrain_world
    WIND_REALISM_SUMMARY = initial.wind
    THERMAL_WEATHER_REALISM_SUMMARY = initial.thermal_weather
    VEHICLE_REALISM_SUMMARY = initial.vehicle
    BATTERY_REALISM_SUMMARY = initial.battery
    SENSOR_REALISM_SUMMARY = initial.sensor
    WORLD_REALISM_SUMMARY = initial.world
    VISIBILITY_REALISM_SUMMARY = initial.visibility
    OPERATIONAL_REALISM_SUMMARY = initial.operational
    MAVLINK_LINK_REALISM_SUMMARY = initial.mavlink_link
...
def main() -> int:
    ...
    with _px4_route_runtime_session(
        _PX4RouteRuntimeLifecycle(
            ...
            reset_runtime_state=_reset_route_runtime_state,
            start_container=_start_container,
            wait_for_home=_wait_for_px4_home,
            initialize_realism=_initialize_route_runtime_realism,
            stop_container=_stop_container,
            mark_cleanup_observed=_mark_cleanup_observed,
        )
    ) as runtime_session:
        run_dir = runtime_session.run_dir
        payload_model_root = runtime_session.payload_model_root
        with TemporaryDirectory() as tmp:
            ...
            if _payload_advisory_recovery_requested(args):
                payload_recovery = _run_payload_recovery_flow(
                    _PayloadRecoveryFlowInputs(
                        ...
                        artifact_dir=run_dir,
                        pickup_pose=pickup_pose,
                        route_target_ned=derive_px4_gazebo_route_target_ned(route),
                        route_altitude_max_m=route.altitude_max_m,
                        payload_action=args.payload_advisory_recovery_action,
                        advisory_ref=args.payload_feasibility_advisory_ref,
                        supervisor_advisory_ref=(PAYLOAD_FEASIBILITY_ADVISORY_REF_PREFIX),
                    ...
                    runtime=_PayloadRecoveryRuntime(
                        terrain_relative_xy_origin=_terrain_relative_xy_origin,
                        ...
                        dispatch_recovery=_dispatch_emergency_recovery,
                        observe_recovery_state=_observe_recovery_state,
                        refresh_realism=lambda: _refresh_payload_recovery_realism(
                            payload_model_root=payload_model_root
                        ),
                        observed_at_iso=lambda: datetime.now(timezone.utc).isoformat(),
                    ),
                )
                _write_recovery_run_artifacts(
                    run_dir=run_dir,
                    summary=payload_recovery.summary,
                    task_artifacts=payload_recovery.updated_task["artifacts"],
                    pose_rows=list(payload_recovery.pose_rows),
                    log_text=_all_logs(),
                    task_db_path=task_db_path,
                )
                print(json.dumps(payload_recovery.summary, indent=2, sort_keys=True))
                _audit_payload_recovery_summary(
                    payload_recovery.summary,
            ...
            def _wait_for_terminal_landing(
                ...
                return _wait_for_z(
                    lambda z, _samples: z <= _landing_z_threshold(pickup_pose),
                    timeout=80.0,
                    phase=phase,
                )

            normal_route = _run_normal_route_flow(
                _NormalRouteFlowInputs(
                    store=store,
                    task_id=task["task_id"],
                    task_db_path=task_db_path,
                    artifact_dir=run_dir,
                    ...
                    live_pose_trace_populated=bool(
                        LIVE_POSE_TRACE_PATH is not None
                        and LIVE_POSE_TRACE_PATH.read_text().strip()
                    ),
                ),
                runtime=_NormalRouteRuntime(
                    terrain_relative_xy_origin=_terrain_relative_xy_origin,
                    wind_compensation_request=_form2a_wind_compensation_request,
                    wind_compensation_xy_offset=_form2a_wind_compensation_xy_offset,
                    wind_feed_forward_xy_mps=_form2a_wind_feed_forward_xy_mps,
                    assert_stream_budget=_assert_planned_route_stream_budget,
                    send_route_with_monitor=_send_route_with_monitor,
                    dispatch_recovery=_dispatch_emergency_recovery,
                    ...
                    pose_sample=_pose_sample,
                    append_pose_row=_append_live_pose_row,
                    collect_route_blocking_observation=(
                        lambda **values: _collect_route_blocking_observation(**values)
                    ),
                    ...
                    project_terminal_realism=lambda **values: (
                        _project_normal_route_terminal_realism(
                            run_dir=run_dir,
                            payload_model_root=payload_model_root,
                            pickup_pose=pickup_pose,
                            route_approval=approval,
                            obstacle_supervisor_loop_requested=(
                                args.mission_os_supervisor_obstacle_loop
                            ),
                            **values,
                        )
                    ),
            ...
            if normal_route.branch == "route_deviation":
                route_deviation = normal_route.deviation_result
                if route_deviation is None:
                    raise RuntimeError("route deviation branch did not return a result")
                _write_recovery_run_artifacts(
                    run_dir=run_dir,
                    summary=route_deviation.summary,
                    task_artifacts=route_deviation.updated_task["artifacts"],
                    pose_rows=list(route_deviation.pose_rows),
                    log_text=_all_logs(),
                    task_db_path=task_db_path,
                )
                print(json.dumps(route_deviation.summary, indent=2, sort_keys=True))
                _audit_route_deviation_recovery_summary(
                    route_deviation.summary,
                ...
                return 0
            if normal_route.summary is None or normal_route.updated_task is None:
                raise RuntimeError("normal route branch did not return final artifacts")
            _write_run_artifacts(
                run_dir=run_dir,
                summary=normal_route.summary,
                task_artifacts=normal_route.updated_task["artifacts"],
                pose_rows=normal_route.fallback_pose_rows,
                log_text=_all_logs(),
                task_db_path=None,
                recorded_at=normal_route.recorded_at,
            )
            print(json.dumps(normal_route.summary, indent=2, sort_keys=True))
            _audit_route_summary(
                normal_route.summary,
                expectations=_RouteAuditExpectations(
                    route_target_x_m=normal_route.target.route_delta_x_m,
                    route_target_y_m=normal_route.target.route_delta_y_m,
                    route_target_z_m=normal_route.target.target_z_m,
                    landing_z_threshold_m=_landing_z_threshold(pickup_pose),
