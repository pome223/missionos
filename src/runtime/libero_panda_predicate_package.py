...
LIBERO_PANDA_EPISODE_RESULT_SCHEMA_VERSION = (
    "missionos_groot_n17_libero_panda_episode_result.v2"
)
...
LIBERO_PANDA_PREDICATE_PACKAGE_ID = (
    "groot_n17_libero_panda_official_sim_task_satisfied"
)
...
LIBERO_PANDA_OUTCOME_CLAIM_SCOPE = (
    "exact_groot_n17_libero_panda_simulator_episode"
)
...
LIBERO_TASK_BDDL_SHA256 = (
    "491835cc2eb6956f7ce3d1ee4e266377116168fe453e6529fe105e3b0333635d"
)
...
LIBERO_PANDA_PREDICATE_PACKAGE_SHA256 = canonical_sha256(
    _PACKAGE_MATERIAL
)
    ...
    def to_material(self) -> dict[str, Any]:
        return {
            ...
            "simulator_step_return_sha256": (
                self.simulator_step_return_sha256
            ),
...
class LIBEROPandaPredicateContent:
    ...
    def to_material(self) -> dict[str, Any]:
        return {
            ...
            "action_chunks": [
                chunk.to_material() for chunk in self.action_chunks
            ],
            "step_applications": [
                step.to_material() for step in self.step_applications
            ],
            "official_runner_episode_ended": (
                self.official_runner_episode_ended
            ),
            "official_runner_episode_success": (
                self.official_runner_episode_success
            ),
                ...
                "env_step_inputs": [
                    {
                        ...
                        "action_chunk_sha256": (
                            step.action_chunk_sha256
                        ),
                        "transformation_names": list(
                            step.transformation_names
                        ),
                ...
                "simulator_step_returns": [
                    {
                        "global_step_index": step.global_step_index,
                        "simulator_step_return_sha256": (
                            step.simulator_step_return_sha256
                        ),
                        "result_observation_sha256": (
                            step.result_observation_sha256
                        ),
                        "official_predicate_result": (
                            step.official_predicate_result
                        ),
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "predicate_package_evaluated": (
                self.predicate_package_evaluated
            ),
            "simulator_step_return_observed": (
                self.simulator_step_return_observed
            ),
            "official_sim_task_success": self.official_sim_task_success,
            "controller_ack_observed": self.controller_ack_observed,
            "controller_ack_verification_status": (
                self.controller_ack_verification_status
            ),
            "readback_satisfies_controller_ack": (
                self.readback_satisfies_controller_ack
            ),
            ...
            "dispatch_authority_created": (
                self.dispatch_authority_created
            ),
            "runtime_effect_requested": self.runtime_effect_requested,
            "operational_closure_created": (
                self.operational_closure_created
            ),
            "physical_execution_invoked": self.physical_execution_invoked,
        }


def libero_panda_predicate_package_binding() -> PredicatePackageBinding:
    return PredicatePackageBinding(
        package_id=LIBERO_PANDA_PREDICATE_PACKAGE_ID,
        package_version=LIBERO_PANDA_PREDICATE_PACKAGE_VERSION,
        content_sha256=LIBERO_PANDA_PREDICATE_PACKAGE_SHA256,
    )
...
def validate_libero_panda_runner_configuration(
    ...
    if not _sha256_is_valid(configuration.controller_configuration_sha256):
        reasons.append(
            "libero_runner_controller_configuration_digest_invalid"
        )
    if (
        isinstance(configuration.action_dim, bool)
        or configuration.action_dim not in (4, 7)
    ):
...
def build_libero_panda_replay_contract(
    *,
    ...
    runner_reasons = validate_libero_panda_runner_configuration(
        runner_configuration
    )
    if runner_reasons:
        raise ValueError(
            "runner configuration is invalid: " + ",".join(runner_reasons)
        )
    ...
    if not str(episode_identity or "").strip():
        raise ValueError(
            "episode identity is required before the first policy request"
        )
    ...
    return FrozenMissionContract(
        ...
        reference_inputs=(
            ...
            ReferenceInput(
                ...
                content_sha256=LIBERO_TASK_PREDICATE_SHA256,
            ),
            ReferenceInput(
        ...
        outcome_claim_spec=OutcomeClaimSpec(
            claim_id=LIBERO_PANDA_OUTCOME_CLAIM_ID,
            statement=(
            ...
            claim_scope=LIBERO_PANDA_OUTCOME_CLAIM_SCOPE,
        ),
        predicate_package=libero_panda_predicate_package_binding(),
...
def evaluate_libero_panda_predicate(
    *,
    ...
    task_satisfied = bool(
        replay.content.step_applications
        and replay.content.step_applications[-1].official_predicate_result
        is True
    )
    return _evaluation(
        contract=contract,
        ...
        reasons=(
            ()
            if task_satisfied
            else ("official_sim_task_predicate_not_satisfied",)
        ),
    ...
    *,
    ...
    expected_references = {
        "approved_runner_configuration": (
            content.runner_configuration.content_sha256
        ),
        ...
        "approved_run_identity": canonical_sha256(
            {"run_identity": content.run_identity}
        ),
    ...
    if (
        contract.quantification_scope.kind
        is not QuantificationScopeKind.ENUMERATED_MEMBERS
        or contract.quantification_scope.member_ids
        != (content.episode_identity,)
    ):
    ...
    if (
        ...
        or observation.source_clock_domain_ref
        != content.source_clock_domain_ref
        or observation.receipt_clock_domain_ref
        != content.receipt_clock_domain_ref
    ):
...
def _lineage_reasons(
    ...
    reasons.extend(
        validate_libero_panda_runner_configuration(
            content.runner_configuration
        )
    )
    if content.task_predicate_sha256 != LIBERO_TASK_PREDICATE_SHA256:
    ...
    if (
        ...
        or content.runtime_action_dim
        != content.runner_configuration.action_dim
    ):
    ...
    for expected_index, chunk in enumerate(chunks):
        ...
        if not _sha256_is_valid(chunk.policy_request_sha256):
            reasons.append(
                f"policy_request_digest_invalid:{chunk.chunk_index}"
            )
        if not _sha256_is_valid(chunk.policy_response_sha256):
            reasons.append(
                f"policy_response_digest_invalid:{chunk.chunk_index}"
            )
        if tuple(field.field_name for field in chunk.fields) != (
            LIBERO_ACTION_FIELDS
        ):
            reasons.append(
                f"policy_action_fields_invalid:{chunk.chunk_index}"
            )
            continue
        for field in chunk.fields:
            if (
                len(field.values)
                != content.runner_configuration.policy_action_horizon
            ):
                reasons.append(
                    f"policy_action_horizon_invalid:{chunk.chunk_index}:"
                    f"{field.field_name}"
                )
            if any(not _finite_number(value) for value in field.values):
                reasons.append(
                    f"policy_action_non_finite:{chunk.chunk_index}:"
                    f"{field.field_name}"
                )
    ...
    for expected_global_index, step in enumerate(steps):
        ...
        if (
            ...
            reasons.append(
                f"simulator_step_flags_invalid:{expected_global_index}"
            )
        terminal_seen = (
        ...
        if chunk is None:
            reasons.append(
                f"simulator_step_chunk_missing:{expected_global_index}"
            )
            continue
        if (
        ...
        expected_chunk_step = expected_step_by_chunk.get(
            step.chunk_index, 0
        )
        if (
            isinstance(step.chunk_step_index, bool)
            or step.chunk_step_index != expected_chunk_step
        ):
            reasons.append(
                f"simulator_chunk_step_order_invalid:{step.chunk_index}"
            )
        expected_step_by_chunk[step.chunk_index] = expected_chunk_step + 1
        if (
            step.chunk_step_index < 0
            or step.chunk_step_index
            >= content.runner_configuration.n_action_steps
        ):
            reasons.append(
                f"simulator_chunk_step_index_invalid:{expected_global_index}"
            )
            continue
        if step.action_chunk_sha256 != chunk.action_chunk_sha256:
            reasons.append(
                f"simulator_step_chunk_digest_mismatch:"
                f"{expected_global_index}"
            )
        ...
        if step.transformation_names != expected_transformations:
            reasons.append(
                f"undeclared_action_transformation:{expected_global_index}"
            )
        ...
        if expected_input is None:
            reasons.append(
                f"simulator_step_input_uncomputable:{expected_global_index}"
            )
        elif canonical_sha256(
            {"env_step_input": list(step.env_step_input)}
        ) != canonical_sha256({"env_step_input": list(expected_input)}):
            reasons.append(
                f"simulator_step_input_mismatch:{expected_global_index}"
            )
        if any(not _finite_number(value) for value in step.env_step_input):
            reasons.append(
                f"simulator_step_input_non_finite:{expected_global_index}"
            )
        if not _sha256_is_valid(step.simulator_step_return_sha256):
            reasons.append(
                f"simulator_step_return_digest_invalid:"
                f"{expected_global_index}"
            )
        if not _sha256_is_valid(step.result_observation_sha256):
            reasons.append(
                f"simulator_result_observation_digest_invalid:"
                f"{expected_global_index}"
            )
    ...
    for chunk in chunks[:-1]:
        if (
            ...
            reasons.append(
                f"nonfinal_policy_chunk_incomplete:{chunk.chunk_index}"
            )
    if steps and any(
        step.official_predicate_result is True for step in steps[:-1]
    ):
    ...
    if (
        ...
        or not steps
        or content.official_runner_episode_success
        is not steps[-1].official_predicate_result
    ):
...
def _expected_env_step_input(
    *,
    ...
    try:
        vector = [
            float(fields[field_name][chunk_step_index])
...
def _evaluation(
    *,
    ...
    return LIBEROPandaPredicateEvaluation(
        ...
        contract_sha256=contract.contract_sha256,
        predicate_package_id=LIBERO_PANDA_PREDICATE_PACKAGE_ID,
        predicate_package_version=LIBERO_PANDA_PREDICATE_PACKAGE_VERSION,
        ...
        raw_action_stream_manifest_sha256=(
            replay.content.raw_action_stream_manifest_sha256
        ),
        env_step_input_stream_manifest_sha256=(
