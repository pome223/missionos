...
## 12. 2026-09-24：WAMの価値を測れる用途へ絞る

次の目的を**点検対象が見やすく撮れる位置を選び、撮り直しや無駄な移動を減らす**
と定めた。これは今後検証する仮説であり、既にWAMの利益が出たという意味ではない。
...
以上から、左右の色を固定して事後学習する前に、対象の手掛かりが履歴にある
点検場面を作る。対象を知る情報が足りなければ短い観測行動を先に比較する。
対象poseが実際に分かる用途では、その基準を残す。画像参照しかない用途では、
全履歴RGB-D投影、目標の見え方を測る共通指標、幾何に基づく観測方策と競わせる。
単純基準の取りこぼしがあり、実行可能な別候補で回復できる条件だけをGPU評価へ
進める。評価指標、開発12例での改善余地確認、未使用24例での比較、計算時間、
事後学習へ進む条件を[次の実験契約](aerial-wam-value-experiment.md)に記した。
